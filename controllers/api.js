...
exports.getFoursquare = function(req, res, next) {
  ...
  let token = _.find(req.user.tokens, {kind: 'foursquare'});
  async.parallel({
...
exports.getTumblr = function(req, res, next) {
  ...
  let token = _.find(req.user.tokens, {kind: 'tumblr'});
...
exports.getFacebook = function(req, res, next) {
  ...
  let token = _.find(req.user.tokens, {kind: 'facebook'});
...
exports.getGithub = function(req, res, next) {
  ...
  let token = _.find(req.user.tokens, {kind: 'github'});
  let github = new Github({token: token.accessToken});
  let repo = github.getRepo('sahat', 'requirejs-library');
...
exports.getNewYorkTimes = function(req, res, next) {
  ...
  request.get({url: 'http://api.nytimes.com/svc/books/v2/lists', qs: query}, function(err, request, body) {
    ...
    let bestsellers = JSON.parse(body);
    res.render('api/nyt', {
...
exports.getTwitter = function(req, res, next) {
  ...
  let token = _.find(req.user.tokens, {kind: 'twitter'});
  let T = new Twit({
...
exports.getSteam = function(req, res, next) {
  ...
  let steamId = '76561197982488301';
  let params = {l: 'english', steamid: steamId, key: process.env.STEAM_KEY};
...
exports.getVenmo = function(req, res, next) {
  ...
  let token = _.find(req.user.tokens, {kind: 'venmo'});
  let query = {access_token: token.accessToken};
...
exports.getLinkedin = function(req, res, next) {
  ...
  let token = _.find(req.user.tokens, {kind: 'linkedin'});
  let linkedin = Linkedin.init(token.accessToken);
...
exports.getInstagram = function(req, res, next) {
  ...
  let token = _.find(req.user.tokens, {kind: 'instagram'});
  ...
  async.parallel([
    function getFinanceStocks(done) {
      Y.YQL('SELECT * FROM yahoo.finance.quote WHERE symbol in ("YHOO", "TSLA", "GOOG", "MSFT")', function(response) {
        let quotes = response.query.results.quote;
        done(null, quotes);
      });
    },
...
exports.getPayPalSuccess = function(req, res) {
  ...
  let paymentDetails = {payer_id: req.query.PayerID};
...
exports.getBitGo = function(req, res, next) {
  ...
  let bitgo = new BitGo.BitGo({env: 'test', accessToken: process.env.BITGO_ACCESS_TOKEN});
  let walletId = req.session.walletId;

  let renderWalletInfo = function(walletId) {
...
exports.getPinterest = function(req, res, next) {
  ...
  let token = _.find(req.user.tokens, {kind: 'pinterest'});
